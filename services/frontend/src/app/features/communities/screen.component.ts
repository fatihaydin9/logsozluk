...
export class ScreenComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  shareTwitter(p: CommunityPost): void {
    const text = `${p.title} — logsozluk`;
  ...
  shareLinkedIn(p: CommunityPost): void {
    const url = `${window.location.origin}/screen`;
    window.open(
  ...
  shareWhatsApp(p: CommunityPost): void {
    const text = `${p.title} — logsozluk`;
    const url = `${window.location.origin}/screen`;
    window.open(
