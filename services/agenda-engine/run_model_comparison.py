...
class ModelTest:
    ...
    async def _call_llm(self, system: str, user: str) -> Tuple[str, int, float]:
        start = datetime.now()
    ...
    async def run_test(self, phase: str, test_type: str, category: str) -> Result:
        ...
        content, tokens, latency = await self._call_llm(system, user)

        if content.startswith("[ERROR"):
            return Result(phase, test_type, category, content, Metrics(), 0, 0, content)

        metrics = self._analyze(content, is_title=(test_type == "title"))
        return Result(phase, test_type, category, content, metrics, tokens, latency)

    async def run_full_test(self, iterations: int = 10):
        print("🚀 gpt-4o-mini Entegrasyon Testi")
        print("=" * 60)
        ...
        for i in range(iterations):
            ...
            print(f"  [{i+1}/{iterations}] {test_type} | {phase} | {category}...", end=" ")

            try:
                result = await self.run_test(phase, test_type, category)
    ...
    def _generate_report(self):
        ...
        report = [
            ...
            "",
            f"- **Test sayısı:** {n}",
            f"- **GIF kullanımı:** {sum(1 for r in valid_results if r.metrics.has_gif)}/{n}",
            f"- **@Mention:** {sum(1 for r in valid_results if r.metrics.has_mention)}/{n}",
            f"- **Conflict:** {sum(1 for r in valid_results if r.metrics.has_conflict)}/{n}",
            f"- **Violation sayısı:** {sum(len(r.metrics.violations) for r in valid_results)}",
            f"- **Ortalama latency:** {sum(r.latency_ms for r in valid_results) / n:.0f}ms",
            "",
            "---",
            "",
            "## ⚠️ Violations",
        ...
        violations = [(r.test_type, r.metrics.violations) for r in valid_results if r.metrics.violations]
        if violations:
            for test_type, v in violations:
                report.append(f"- {test_type}: {', '.join(v)}")
        else:
            report.append("*Violation yok*")
        ...
        for r in valid_results[:5]:
            ...
            report.extend([
                f"### {emoji} {r.test_type.upper()} [{r.category}]",
                "",
                f"> {r.content}",
                "",
                f"*GIF: {r.metrics.gif_count} | @: {r.metrics.mention_count} | Emoji: {r.metrics.emoji_count} | {r.latency_ms:.0f}ms | {flags}*",
                "",
                "---",
                ""
            ])
...
async def main():
    test = ModelTest()
    await test.run_full_test(iterations=10)
